...
import FactureAcompteLinesEditor from './FactureAcompteLinesEditor';

const FactureAcompteModal = ({ 
  isOpen, 
  mode = 'add',
  formData,
  errors,
  ...
  const [showFinancementExterne, setShowFinancementExterne] = useState(false);
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-5xl max-h-[95vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">
            {mode === 'add' ? 'Ajouter une facture d\'acompte' : 'Modifier la facture d\'acompte'}
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(95vh-8rem)]">
          <div className="space-y-6">
            ...
            {/* Catégorie et Rapport personnalisé */}
            <div className="grid grid-cols-2 gap-4">
              {/* Catégorie */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  ...
                  Date de la facture d'acompte <span className="text-red-500">*</span>
                </label>
                <input
                  type="date"
                  value={formData.dateFactureAcompte}
                  onChange={(e) => onUpdateField('dateFactureAcompte', e.target.value)}
                  className={`w-full border rounded-md px-3 py-2 text-sm ${
                    errors.dateFactureAcompte ? 'border-red-300' : 'border-gray-300'
                  } focus:outline-none focus:ring-1 focus:ring-blue-500`}
                />
                {errors.dateFactureAcompte && (
                  <p className="mt-1 text-xs text-red-600">{errors.dateFactureAcompte}</p>
                )}
              </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                placeholder="Commentaire visible sur la facture d'acompte..."
              />
            </div>

            {/* Lignes de facturation */}
            <div>
              <FactureAcompteLinesEditor
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <h3 className="text-sm font-medium text-gray-900 mb-4">Calculs</h3>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                ...
                {/* Financement externe - Accordéon */}
                <div>
                  <button
                    type="button"
                    onClick={() => setShowFinancementExterne(!showFinancementExterne)}
        ...
        <div className="flex justify-between items-center px-6 py-4 border-t border-gray-200 bg-gray-50">
          ...
          <div className="flex items-center space-x-3">
            ...
            >
              {loading ? 'Enregistrement...' : 'Acompte en cours'}
            </button>
            <button
            ...
            >
              {loading ? 'Enregistrement...' : 'Acompte payé'}
            </button>
            <button
            ...
            >
              {loading ? 'Enregistrement...' : 'Acompte impayé'}
            </button>
          </div>
        </div>
      </div>
...
export default FactureAcompteModal;