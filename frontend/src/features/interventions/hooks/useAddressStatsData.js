...
// Données mock pour la simulation - données par adresse
const mockAddressInterventions = [
  {
    id: 1,
    date: '2025-07-01',
...
export const useAddressStatsData = ({ dateRange, timeGroup, dataChoice }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setRawData(mockAddressInterventions);
      setLoading(false);
    }, 500);
  ...
  // Grouper les données par adresse
  const groupedByAddress = useMemo(() => {
    ...
    filteredData.forEach(item => {
      ...
      groups[item.adresse].interventions += 1;
      groups[item.adresse].dureeMinutes += item.dureeMinutes;
      groups[item.adresse].budgetTemps += item.budgetTemps;
      groups[item.adresse].budgetReel += item.budgetReel;
      groups[item.adresse].tarifsHoraires.push({
        tarif: item.tarifHoraire,
        heures: item.dureeMinutes / 60
      });
    });

    // Calculer tarif moyen pondéré par adresse
    Object.keys(groups).forEach(key => {
      // Calculer le tarif horaire moyen pondéré pour cette adresse
      const totalHeuresAdresse = groups[key].dureeMinutes / 60;
      if (totalHeuresAdresse > 0) {
        const tarifPondere = groups[key].tarifsHoraires.reduce((sum, item) => {
          return sum + (item.tarif * item.heures);
        }, 0);
        groups[key].tarifHoraire = Math.round((tarifPondere / totalHeuresAdresse) * 100) / 100;
      } else {
  ...
  const tarifHoraire = useMemo(() => {
    ...
    // Pondérer par les heures travaillées de chaque adresse
    const totalHeures = totalData.dureeMinutes / 60;
    if (totalHeures === 0) return 37.50;
    
    const tarifPondere = tableData.reduce((sum, item) => {
      const heuresAdresse = item.dureeMinutes / 60;
      return sum + (item.tarifHoraire * heuresAdresse);
    }, 0);
  ...
  const exportData = (format = 'csv') => {
    if (format === 'csv') {
      ...
      link.setAttribute('download', `temps-travaille-adresses-${format(new Date(), 'yyyy-MM-dd')}.csv`);
