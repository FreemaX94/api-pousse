...
// Données mock pour la simulation - données par provenance de clients
const mockClientOriginInterventions = [
  {
    id: 1,
    date: '2025-07-01',
    ...
    client: 'Singular',
    provenance: 'Recommandation client',
  ...
  },
  {
    ...
    provenance: 'Prospection téléphonique',
  ...
  },
  {
    ...
    fonction: 'Technicien espaces verts',
    client: 'L\'OREAL',
    provenance: 'Salon professionnel',
  ...
  },
  {
    ...
    provenance: 'Partenaire commercial',
  ...
  },
  {
    ...
    fonction: 'Technicien espaces verts',
    client: 'ORANGE',
    provenance: 'Référencement naturel',
  ...
  },
  {
    ...
    fonction: 'Chef d\'équipe',
    client: 'Station F',
    provenance: 'Prospection directe',
  ...
  },
  {
    ...
    fonction: 'Jardinier paysagiste',
    client: 'AMAZON',
    provenance: 'E-mailing',
    adresse: 'AMAZON - 67 Boulevard du Général Leclerc, 92110, Clichy',
    dureeMinutes: 115,
    budgetTemps: 71.85,
    budgetReel: 80.50,
    statut: 'effectue',
  ...
  },
  {
    ...
    client: 'FACEBOOK',
    provenance: 'Bouche-à-oreille',
    adresse: 'FACEBOOK - 6 Rue Ménars, 75002, Paris',
    dureeMinutes: 185,
    budgetTemps: 115.65,
    budgetReel: 129.50,
    statut: 'effectue',
...
export const useClientOriginData = ({ dateRange, timeGroup, dataChoice }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setRawData(mockClientOriginInterventions);
      setLoading(false);
    }, 500);
  ...
  // Grouper les données par provenance
  const groupedByOrigin = useMemo(() => {
    ...
    filteredData.forEach(item => {
      if (!groups[item.provenance]) {
        groups[item.provenance] = {
      ...
      groups[item.provenance].adresses.add(item.adresse);
      groups[item.provenance].interventions += 1;
      groups[item.provenance].dureeMinutes += item.dureeMinutes;
      groups[item.provenance].budgetTemps += item.budgetTemps;
      groups[item.provenance].budgetReel += item.budgetReel;
      groups[item.provenance].tarifsHoraires.push({
        tarif: item.tarifHoraire,
        heures: item.dureeMinutes / 60
      });
    });

    // Convertir les Set en nombre et calculer tarif moyen pondéré par provenance
    Object.keys(groups).forEach(key => {
      groups[key].adresses = groups[key].adresses.size;
      
      // Calculer le tarif horaire moyen pondéré pour cette provenance
      const totalHeuresProvenance = groups[key].dureeMinutes / 60;
      if (totalHeuresProvenance > 0) {
        const tarifPondere = groups[key].tarifsHoraires.reduce((sum, item) => {
          return sum + (item.tarif * item.heures);
        }, 0);
        groups[key].tarifHoraire = Math.round((tarifPondere / totalHeuresProvenance) * 100) / 100;
      } else {
  ...
  const tableData = useMemo(() => {
    return Object.values(groupedByOrigin).map(group => ({
  ...
  const tarifHoraire = useMemo(() => {
    ...
    // Pondérer par les heures travaillées de chaque provenance
    const totalHeures = totalData.dureeMinutes / 60;
    if (totalHeures === 0) return 37.50;
    
    const tarifPondere = tableData.reduce((sum, item) => {
      const heuresProvenance = item.dureeMinutes / 60;
      return sum + (item.tarifHoraire * heuresProvenance);
    }, 0);
  ...
  const exportData = (format = 'csv') => {
    if (format === 'csv') {
      ...
      link.setAttribute('download', `temps-travaille-provenances-${format(new Date(), 'yyyy-MM-dd')}.csv`);
