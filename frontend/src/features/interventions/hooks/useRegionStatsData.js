...
// Données mock pour la simulation - données par région
const mockRegionInterventions = [
  {
    id: 1,
    date: '2025-07-01',
...
export const useRegionStatsData = ({ dateRange, timeGroup, dataChoice }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setRawData(mockRegionInterventions);
      setLoading(false);
    }, 500);
  ...
  const groupedByRegion = useMemo(() => {
    ...
    filteredData.forEach(item => {
      const key = `${item.pays}_${item.region}`;
    ...
    // Convertir les Set en nombre et calculer tarif moyen pondéré par région
    Object.keys(groups).forEach(key => {
      groups[key].adresses = groups[key].adresses.size;
      
      // Calculer le tarif horaire moyen pondéré pour cette région
      const totalHeuresRegion = groups[key].dureeMinutes / 60;
      if (totalHeuresRegion > 0) {
        const tarifPondere = groups[key].tarifsHoraires.reduce((sum, item) => {
          return sum + (item.tarif * item.heures);
        }, 0);
        groups[key].tarifHoraire = Math.round((tarifPondere / totalHeuresRegion) * 100) / 100;
      } else {
  ...
  const tableData = useMemo(() => {
    return Object.values(groupedByRegion).map(group => ({
      pays: group.pays,
      region: group.region,
      type: group.type,
  ...
  const tarifHoraire = useMemo(() => {
    ...
    // Pondérer par les heures travaillées de chaque région
    const totalHeures = totalData.dureeMinutes / 60;
    if (totalHeures === 0) return 37.50;
    
    const tarifPondere = tableData.reduce((sum, item) => {
      const heuresRegion = item.dureeMinutes / 60;
      return sum + (item.tarifHoraire * heuresRegion);
    }, 0);
  ...
  const exportData = (format = 'csv') => {
    if (format === 'csv') {
      ...
      link.setAttribute('download', `temps-travaille-regions-${format(new Date(), 'yyyy-MM-dd')}.csv`);
