...
// Données mock pour la simulation - données par client
const mockClientInterventions = [
  {
    id: 1,
    date: '2025-07-01',
...
export const useClientTimeData = ({ dateRange, timeGroup, dataChoice }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setRawData(mockClientInterventions);
      setLoading(false);
    }, 500);
  ...
  const groupedByClient = useMemo(() => {
    ...
    filteredData.forEach(item => {
      ...
      groups[item.client].adresses.add(item.adresse);
      groups[item.client].interventions += 1;
      groups[item.client].dureeMinutes += item.dureeMinutes;
      groups[item.client].budgetTemps += item.budgetTemps;
      groups[item.client].budgetReel += item.budgetReel;
      groups[item.client].tarifsHoraires.push({
        tarif: item.tarifHoraire,
        heures: item.dureeMinutes / 60
      });
    });

    // Convertir les Set en nombre et calculer tarif moyen pondéré par client
    Object.keys(groups).forEach(key => {
      groups[key].adresses = groups[key].adresses.size;
      
      // Calculer le tarif horaire moyen pondéré pour ce client
      const totalHeuresClient = groups[key].dureeMinutes / 60;
      if (totalHeuresClient > 0) {
        const tarifPondere = groups[key].tarifsHoraires.reduce((sum, item) => {
          return sum + (item.tarif * item.heures);
        }, 0);
        groups[key].tarifHoraire = Math.round((tarifPondere / totalHeuresClient) * 100) / 100;
      } else {
  ...
  const tableData = useMemo(() => {
    return Object.values(groupedByClient).map(group => ({
      client: group.client,
      type: group.type,
  ...
  const tarifHoraire = useMemo(() => {
    ...
    // Pondérer par les heures travaillées de chaque client
    const totalHeures = totalData.dureeMinutes / 60;
    if (totalHeures === 0) return 37.50;
    
    const tarifPondere = tableData.reduce((sum, item) => {
      const heuresClient = item.dureeMinutes / 60;
      return sum + (item.tarifHoraire * heuresClient);
    }, 0);
  ...
  const exportData = (format = 'csv') => {
    if (format === 'csv') {
      ...
      link.setAttribute('download', `temps-travaille-clients-${format(new Date(), 'yyyy-MM-dd')}.csv`);
