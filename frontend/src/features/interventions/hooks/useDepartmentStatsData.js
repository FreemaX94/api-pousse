...
// Données mock pour la simulation - données par département
const mockDepartmentInterventions = [
  {
    id: 1,
    date: '2025-07-01',
...
export const useDepartmentStatsData = ({ dateRange, timeGroup, dataChoice, measure = 'heures' }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setRawData(mockDepartmentInterventions);
      setLoading(false);
    }, 500);
  ...
  const groupedByDepartment = useMemo(() => {
    ...
    filteredData.forEach(item => {
      const key = `${item.pays}_${item.region}_${item.departement}`;
    ...
    // Convertir les Set en nombre et calculer tarif moyen pondéré par département
    Object.keys(groups).forEach(key => {
      groups[key].adresses = groups[key].adresses.size;
      
      // Calculer le tarif horaire moyen pondéré pour ce département
      const totalHeuresDepartement = groups[key].dureeMinutes / 60;
      if (totalHeuresDepartement > 0) {
        const tarifPondere = groups[key].tarifsHoraires.reduce((sum, item) => {
          return sum + (item.tarif * item.heures);
        }, 0);
        groups[key].tarifHoraire = Math.round((tarifPondere / totalHeuresDepartement) * 100) / 100;
      } else {
  ...
  const tableData = useMemo(() => {
    return Object.values(groupedByDepartment).map(group => ({
      pays: group.pays,
      region: group.region,
  ...
  const tarifHoraire = useMemo(() => {
    ...
    // Pondérer par les heures travaillées de chaque département
    const totalHeures = totalData.dureeMinutes / 60;
    if (totalHeures === 0) return 37.50;
    
    const tarifPondere = tableData.reduce((sum, item) => {
      const heuresDepartement = item.dureeMinutes / 60;
      return sum + (item.tarifHoraire * heuresDepartement);
    }, 0);
  ...
  const exportData = (format = 'csv') => {
    if (format === 'csv') {
      ...
      link.setAttribute('download', `temps-travaille-departements-${format(new Date(), 'yyyy-MM-dd')}.csv`);
