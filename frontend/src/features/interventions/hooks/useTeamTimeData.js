...
// Données mock pour la simulation - données par équipe
const mockTeamInterventions = [
  {
    id: 1,
    date: '2025-07-01',
...
export const useTeamTimeData = ({ dateRange, timeGroup, dataChoice }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setRawData(mockTeamInterventions);
      setLoading(false);
    }, 500);
  ...
  const groupedByTeam = useMemo(() => {
    ...
    filteredData.forEach(item => {
      ...
      groups[item.equipe].adresses.add(item.adresse);
      groups[item.equipe].collaborateurs.add(item.collaborateur);
      groups[item.equipe].interventions += 1;
      groups[item.equipe].dureeMinutes += item.dureeMinutes;
      groups[item.equipe].budgetTemps += item.budgetTemps;
      groups[item.equipe].budgetReel += item.budgetReel;
      groups[item.equipe].tarifsHoraires.push({
        tarif: item.tarifHoraire,
        heures: item.dureeMinutes / 60
      });
    });

    // Convertir les Set en nombre et calculer tarif moyen pondéré par équipe
    Object.keys(groups).forEach(key => {
      groups[key].adresses = groups[key].adresses.size;
      groups[key].nombreCollaborateurs = groups[key].collaborateurs.size;
      
      // Calculer le tarif horaire moyen pondéré pour cette équipe
      const totalHeuresEquipe = groups[key].dureeMinutes / 60;
      if (totalHeuresEquipe > 0) {
        const tarifPondere = groups[key].tarifsHoraires.reduce((sum, item) => {
          return sum + (item.tarif * item.heures);
        }, 0);
        groups[key].tarifHoraire = Math.round((tarifPondere / totalHeuresEquipe) * 100) / 100;
      } else {
        groups[key].tarifHoraire = 37.50; // Valeur par défaut
      }
      
      delete groups[key].collaborateurs;
  ...
  const tableData = useMemo(() => {
    return Object.values(groupedByTeam).map(group => ({
      equipe: group.equipe,
      type: group.type,
      ...
      nombreCollaborateurs: group.nombreCollaborateurs
    }));
  }, [groupedByTeam]);
  ...
  const tarifHoraire = useMemo(() => {
    ...
    // Pondérer par les heures travaillées de chaque équipe
    const totalHeures = totalData.dureeMinutes / 60;
    if (totalHeures === 0) return 37.50;
    
    const tarifPondere = tableData.reduce((sum, item) => {
      const heuresEquipe = item.dureeMinutes / 60;
      return sum + (item.tarifHoraire * heuresEquipe);
    }, 0);
  ...
  const exportData = (format = 'csv') => {
    if (format === 'csv') {
      ...
      link.setAttribute('download', `temps-travaille-equipes-${format(new Date(), 'yyyy-MM-dd')}.csv`);
