...
import { useContractStatsData } from '../hooks/useContractStatsData';
...
const ContractStats = () => {
  ...
  const [activeStatsTab, setActiveStatsTab] = useState('Contrat');
  ...
  // Fonction pour obtenir l'icône selon le type de contrat
  const getContractIcon = (contrat) => {
    if (contrat.includes('Premium') || contrat.includes('Prestige')) {
      return '⭐';
    } else if (contrat.includes('Urgence') || contrat.includes('Dépannage')) {
      return '🚨';
    } else if (contrat.includes('Annuel')) {
      return '📅';
    } else if (contrat.includes('Saisonnière') || contrat.includes('Trimestriel')) {
      return '🌱';
    } else if (contrat.includes('Mensuelle')) {
      return '📆';
    } else if (contrat.includes('Ponctuel') || contrat.includes('Événementiel')) {
      return '⚡';
    } else if (contrat.includes('Multisite') || contrat.includes('Corporatif')) {
      return '🏢';
    } else if (contrat.includes('Plantes') || contrat.includes('Intérieur')) {
      return '🪴';
    } else {
      return '📋';
    }
      ...
      <div className="flex-1 overflow-auto">
        <div className="p-6 space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <h3 className="text-lg font-semibold mb-4">Heures travaillées par {timeGroup.toLowerCase()} - Contrats</h3>
            <div className="h-80">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
              <h3 className="text-lg font-semibold">Détails par contrat</h3>
              <button
                  ...
                  <tr>
                    {visibleColumns.numeroContrat && (
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <SortButton column="numeroContrat">N° Contrat</SortButton>
                      </th>
                    )}
                    {visibleColumns.contrat && (
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <SortButton column="contrat">Contrat</SortButton>
                      </th>
                    )}
                    {visibleColumns.type && (
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                    ...
                    {visibleColumns.dureeHeures && (
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <SortButton column="dureeMinutes">Durée</SortButton>
                      </th>
                    )}
                    ...
                    <tr key={index} className="hover:bg-gray-50">
                      {visibleColumns.numeroContrat && (
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          <button className="font-mono text-blue-600 hover:text-blue-900 hover:underline">
                            {item.numeroContrat}
                          </button>
                        </td>
                      )}
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          <div className="flex items-center">
                            <div className="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center mr-3">
                              <span className="text-lg">{getContractIcon(item.contrat)}</span>
                            </div>
                            <div className="flex items-center space-x-2">
                              <button className="font-medium text-green-600 hover:text-green-900 hover:underline max-w-xs truncate">
                                {item.contrat}
                              </button>
