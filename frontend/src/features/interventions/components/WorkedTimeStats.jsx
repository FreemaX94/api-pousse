...
import { useWorkedTimeData } from '../hooks/useWorkedTimeData';
import UserTimeStats from './UserTimeStats';
import RoleTimeStats from './RoleTimeStats';
import TeamTimeStats from './TeamTimeStats';
import ClientTimeStats from './ClientTimeStats';
import ClientOriginStats from './ClientOriginStats';
import ContractStats from './ContractStats';
import AddressStats from './AddressStats';
import ActionStats from './ActionStats';
import CustomFieldStats from './CustomFieldStats';
import CountryStats from './CountryStats';
import RegionStats from './RegionStats';
import DepartmentStats from './DepartmentStats';
import CityStats from './CityStats';

const WorkedTimeStats = () => {
  ...
  const [activeStatsTab, setActiveStatsTab] = useState('General');
  ...
  const statsTabsConfig = [
    ...
    { key: 'Fonction des utilisateurs', label: 'Fonction des utilisateurs', icon: UsersIcon },
    { key: 'Equipe', label: 'Équipe', icon: UsersIcon },
    { key: 'Client', label: 'Client', icon: BuildingOfficeIcon },
    { key: 'Provenance', label: 'Provenance', icon: TagIcon },
  ...
  const sortedData = getSortedData(currentItems);
  ...
  if (activeStatsTab === 'Collaborateur') {
    return <UserTimeStats />;
  }

  if (activeStatsTab === 'Fonction des utilisateurs') {
    return <RoleTimeStats />;
  }

  if (activeStatsTab === 'Equipe') {
    return <TeamTimeStats />;
  }

  if (activeStatsTab === 'Client') {
    return <ClientTimeStats />;
  }

  if (activeStatsTab === 'Provenance') {
    return <ClientOriginStats />;
  }

  if (activeStatsTab === 'Contrat') {
    return <ContractStats />;
  }

  if (activeStatsTab === 'Adresse') {
    return <AddressStats />;
  }

  if (activeStatsTab === 'Action') {
    return <ActionStats />;
  }

  if (activeStatsTab === 'Champ personnalise') {
    return <CustomFieldStats />;
  }

  if (activeStatsTab === 'Pays') {
    return <CountryStats />;
  }

  if (activeStatsTab === 'Region') {
    return <RegionStats />;
  }

  if (activeStatsTab === 'Departement') {
    return <DepartmentStats />;
  }

  if (activeStatsTab === 'Ville') {
    return <CityStats />;
  }
  ...
  return (
    <div className="flex h-full bg-gray-50">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h3 className="text-lg font-semibold mb-4">Heures travaillées par {timeGroup.toLowerCase()}</h3>
        <div className="h-80">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          <h3 className="text-lg font-semibold">Détails par période</h3>
          <button
        ...
        <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
          <div className="text-sm text-gray-700">
            Affichage de {startIndex + 1} à {Math.min(endIndex, tableData.length)} sur {tableData.length} résultats
          </div>
...
export default WorkedTimeStats;