...
import { useCustomFieldStatsData } from '../hooks/useCustomFieldStatsData';
...
const CustomFieldStats = () => {
  ...
  const [activeStatsTab, setActiveStatsTab] = useState('Champ personnalise');
  
  const [visibleColumns, setVisibleColumns] = useState({
    champPersonnalise: true,
    typeChamp: true,
    type: true,
    adresses: true,
    interventions: true,
            ...
            {Object.entries(visibleColumns).map(([key, value]) => (
              <label key={key} className="flex items-center">
                ...
                <span className="text-sm">
                  {key === 'champPersonnalise' && 'Champ personnalisé'}
                  {key === 'typeChamp' && 'Type de champ'}
                  {key === 'type' && 'Type'}
  ...
  // Fonction pour obtenir l'icône selon le type de champ personnalisé
  const getCustomFieldIcon = (typeChamp) => {
    if (typeChamp.includes('Priorité')) {
      return '⚡';
    } else if (typeChamp.includes('Type de service')) {
      return '🔧';
    } else if (typeChamp.includes('Environnement')) {
      return '🌿';
    } else if (typeChamp.includes('Certification')) {
      return '🏆';
    } else {
      return '🏷️';
    }
      ...
      <div className="flex-1 overflow-auto">
        <div className="p-6 space-y-6">
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  <p className="text-sm text-amber-700">
                    Le rapport devient non proportionnel si plusieurs champs personnalisés du même type ont été associées à une même intervention.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <h3 className="text-lg font-semibold mb-4">Heures travaillées par {timeGroup.toLowerCase()} - Champs personnalisés</h3>
            <div className="h-80">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
              <h3 className="text-lg font-semibold">Détails par champ personnalisé</h3>
              <button
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    {visibleColumns.champPersonnalise && (
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <SortButton column="champPersonnalise">Champ personnalisé</SortButton>
                      </th>
                    )}
                    {visibleColumns.typeChamp && (
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <SortButton column="typeChamp">Type de champ</SortButton>
                      </th>
                    )}
                    {visibleColumns.type && (
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          <div className="flex items-center">
                            <div className="w-8 h-8 bg-indigo-100 rounded-full flex items-center justify-center mr-3">
                              <span className="text-lg">{getCustomFieldIcon(item.typeChamp)}</span>
                            </div>
                            <div className="flex items-center space-x-2">
                              <button className="font-medium text-indigo-600 hover:text-indigo-900 hover:underline">
                                {item.champPersonnalise}
                              </button>
...
export default CustomFieldStats;