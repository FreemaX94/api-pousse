...
import { useRegionStatsData } from '../hooks/useRegionStatsData';
...
const RegionStats = () => {
  ...
  const [activeStatsTab, setActiveStatsTab] = useState('Region');
  ...
  // Fonction pour obtenir l'icône de la région
  const getRegionIcon = (region, pays) => {
    // Icônes spécifiques par région
    if (region.includes('Île-de-France')) {
      return '🗼'; // Tour Eiffel pour Paris
    } else if (region.includes('Auvergne-Rhône-Alpes')) {
      return '⛷️'; // Ski pour les Alpes
    } else if (region.includes('Occitanie')) {
      return '✈️'; // Avion pour Toulouse/Airbus
    } else if (region.includes('Nouvelle-Aquitaine')) {
      return '🍷'; // Vin pour Bordeaux
    } else if (region.includes('Hauts-de-France')) {
      return '🏭'; // Industrie pour le Nord
    } else if (region.includes('Bretagne')) {
      return '🌊'; // Océan pour la Bretagne
    } else if (region.includes('Pays de la Loire')) {
      return '🏰'; // Château pour la Loire
    } else if (region.includes('Grand Est')) {
      return '🍾'; // Champagne pour Reims
    } else if (region.includes('Bruxelles')) {
      return '🇪🇺'; // UE pour Bruxelles
    } else if (region.includes('Flandre')) {
      return '🍺'; // Bière pour la Flandre
    } else if (region.includes('Zurich')) {
      return '🏦'; // Banque pour Zurich
    } else if (region.includes('Vaud')) {
      return '🏔️'; // Montagne pour Vaud
    } else if (region.includes('Luxembourg')) {
      return '💰'; // Argent pour Luxembourg
    } else {
      return '📍'; // Localisation générique
    }
  };
      ...
      <div className="flex-1 overflow-auto">
        <div className="p-6 space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <h3 className="text-lg font-semibold mb-4">Heures travaillées par {timeGroup.toLowerCase()} - Régions</h3>
            <div className="h-80">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
              <h3 className="text-lg font-semibold">Détails par région</h3>
              <button
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          <div className="flex items-center">
                            <div className="w-8 h-8 bg-rose-100 rounded-full flex items-center justify-center mr-3">
                              <span className="text-lg">{getRegionIcon(item.region, item.pays)}</span>
                            </div>
                            <div className="flex items-center space-x-2">
                              <button className="font-medium text-rose-600 hover:text-rose-900 hover:underline">
                                {item.region}
                              </button>
