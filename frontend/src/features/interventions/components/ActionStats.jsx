...
import { useActionStatsData } from '../hooks/useActionStatsData';
...
const ActionStats = () => {
  ...
  const [activeStatsTab, setActiveStatsTab] = useState('Action');
  const [distributionMode, setDistributionMode] = useState('temps'); // 'temps' ou 'cases'
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      const unit = distributionMode === 'temps' ? 'h' : '';
      return (
            ...
            {Object.entries(visibleColumns).map(([key, value]) => (
              <label key={key} className="flex items-center">
                ...
                <span className="text-sm">
                  ...
                  {key === 'action_export' && 'Action (export)'}
                </span>
              </label>
            ))}
          </div>
  ...
  const getActionIcon = (action) => {
    if (action.includes('Arrosage')) {
      return '💧';
    } else if (action.includes('Taille')) {
      return '✂️';
    } else if (action.includes('Nettoyage')) {
      return '🧽';
    } else if (action.includes('Fertilisation')) {
      return '🌱';
    } else if (action.includes('Rempotage')) {
      return '🪴';
    } else if (action.includes('Débroussaillage')) {
      return '🌿';
    } else if (action.includes('Traitement')) {
      return '🧪';
    } else if (action.includes('Entretien')) {
      return '🔧';
    } else {
      return '⚡';
    }
    ...
    <div className="flex h-full bg-gray-50">
      ...
      <div className="flex-1 overflow-auto">
        <div className="p-6 space-y-6">
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  <p className="text-sm text-amber-700">
                    Le rapport dépend des heures indiquées pour chaque action des interventions.
                  </p>
                </div>
              </div>
            </div>
          ...
          {/* Switch de répartition + Graphique */}
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold">Heures travaillées par {timeGroup.toLowerCase()} - Actions</h3>
              
              {/* Switch de répartition */}
              <div className="flex border border-gray-300 rounded-md overflow-hidden">
                <button
                  onClick={() => setDistributionMode('temps')}
                  className={`px-4 py-2 text-sm font-medium transition-colors ${
                    distributionMode === 'temps'
                      ? 'bg-yellow-500 text-white'
                      : 'bg-white text-gray-700 hover:bg-gray-50'
                  }`}
                >
                  Répartition par temps indiqués par actions
                </button>
                <button
                  onClick={() => setDistributionMode('cases')}
                  className={`px-4 py-2 text-sm font-medium transition-colors ${
                    distributionMode === 'cases'
                      ? 'bg-yellow-500 text-white'
                      : 'bg-white text-gray-700 hover:bg-gray-50'
                  }`}
                >
                  Répartition par case cochée
                </button>
              </div>
            </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData}>
                  ...
                  <YAxis 
                    tick={{ fontSize: 12 }}
                    stroke="#6b7280"
                    tickFormatter={(value) => distributionMode === 'temps' ? `${value}h` : `${value}`}
                  />
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
              <h3 className="text-lg font-semibold">Détails par action</h3>
              <button
                      ...
                      {visibleColumns.action && (
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          <div className="flex items-center">
                            ...
                            <div className="flex items-center space-x-2">
                              <button className="font-medium text-yellow-600 hover:text-yellow-900 hover:underline">
                                {item.action}
                              </button>
