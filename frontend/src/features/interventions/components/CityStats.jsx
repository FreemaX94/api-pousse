...
import { useCityStatsData } from '../hooks/useCityStatsData';
...
const CityStats = () => {
  ...
  const [activeStatsTab, setActiveStatsTab] = useState('Ville');
  ...
  // Fonction pour obtenir l'icône de la ville
  const getCityIcon = (ville, pays) => {
    // Icônes spécifiques par ville
    if (ville.includes('Paris')) {
      return '🗼'; // Tour Eiffel
    } else if (ville.includes('Lyon')) {
      return '🦁'; // Lion de Lyon
    } else if (ville.includes('Toulouse')) {
      return '✈️'; // Aéronautique
    } else if (ville.includes('Blagnac')) {
      return '✈️'; // Airbus
    } else if (ville.includes('Bordeaux')) {
      return '🍷'; // Vin
    } else if (ville.includes('Marseille')) {
      return '⛵'; // Port
    } else if (ville.includes('Lille') || ville.includes('Loos') || ville.includes('Villeneuve-d\'Ascq')) {
      return '🏭'; // Industrie
    } else if (ville.includes('Rennes')) {
      return '🌊'; // Bretagne
    } else if (ville.includes('Angers')) {
      return '🏰'; // Châteaux
    } else if (ville.includes('Clermont-Ferrand')) {
      return '🌋'; // Volcans
    } else if (ville.includes('Grenoble')) {
      return '⛷️'; // Ski
    } else if (ville.includes('Metz-Tessy') || ville.includes('Évian-les-Bains')) {
      return '🏔️'; // Montagnes
    } else if (ville.includes('Rueil-Malmaison')) {
      return '🏢'; // Business
    } else if (ville.includes('Épernay')) {
      return '🍾'; // Champagne
    } else if (ville.includes('Bruxelles')) {
      return '🇪🇺'; // UE
    } else if (ville.includes('Gent')) {
      return '🍺'; // Bière flamande
    } else if (ville.includes('Zürich')) {
      return '🏦'; // Banques
    } else if (ville.includes('Vevey')) {
      return '🍫'; // Nestlé/Chocolat
    } else if (ville.includes('Luxembourg')) {
      return '💰'; // Finance
    } else {
      return '🏙️'; // Ville générique
    }
  };
      ...
      <div className="flex-1 overflow-auto">
        <div className="p-6 space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <h3 className="text-lg font-semibold mb-4">{getMeasureLabel()} {timeGroup.toLowerCase()} - Villes</h3>
            <div className="h-80">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
              <h3 className="text-lg font-semibold">Détails par ville</h3>
              <button
                ...
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    {visibleColumns.ville && (
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <SortButton column="ville">Ville</SortButton>
                      </th>
                    )}
                    {visibleColumns.type && (
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          <div className="flex items-center">
                            <div className="w-8 h-8 bg-teal-100 rounded-full flex items-center justify-center mr-3">
                              <span className="text-lg">{getCityIcon(item.ville, item.pays)}</span>
                            </div>
                            <div className="flex items-center space-x-2">
                              <button className="font-medium text-teal-600 hover:text-teal-900 hover:underline">
                                {item.villeComplete}
                              </button>
