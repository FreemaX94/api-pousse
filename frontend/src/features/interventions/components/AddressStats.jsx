...
import { useAddressStatsData } from '../hooks/useAddressStatsData';
...
const AddressStats = () => {
  ...
  const [activeStatsTab, setActiveStatsTab] = useState('Adresse');
  ...
  // Fonction pour obtenir l'icône selon l'arrondissement/ville
  const getLocationIcon = (adresse) => {
    if (adresse.includes('75001') || adresse.includes('75002') || adresse.includes('75003') || adresse.includes('75004')) {
      return '🏛️'; // Centre historique
    } else if (adresse.includes('75008') || adresse.includes('75009') || adresse.includes('75016') || adresse.includes('75017')) {
      return '💼'; // Quartiers d'affaires
    } else if (adresse.includes('75013') || adresse.includes('75015') || adresse.includes('75005') || adresse.includes('75007')) {
      return '🏢'; // Quartiers résidentiels/mixtes
    } else if (adresse.includes('92110') || adresse.includes('92400') || adresse.includes('92130')) {
      return '🌆'; // Banlieue proche
    } else if (adresse.includes('Champs-Élysées') || adresse.includes('Haussmann') || adresse.includes('Lincoln')) {
      return '✨'; // Quartiers prestigieux
    } else {
      return '📍'; // Localisation générique
    }
  };

  // Fonction pour extraire le nom court de l'adresse
  const getShortAddress = (adresse) => {
    const parts = adresse.split(' - ');
    return parts.length > 1 ? parts[1] : adresse;
  };
      ...
      <div className="flex-1 overflow-auto">
        <div className="p-6 space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <h3 className="text-lg font-semibold mb-4">Heures travaillées par {timeGroup.toLowerCase()} - Adresses</h3>
            <div className="h-80">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
              <h3 className="text-lg font-semibold">Détails par adresse</h3>
              <button
                  ...
                  <tr>
                    {visibleColumns.adresse && (
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <SortButton column="adresse">Adresse</SortButton>
                      </th>
                    )}
                    {visibleColumns.type && (
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          <div className="flex items-center">
                            <div className="w-8 h-8 bg-purple-100 rounded-full flex items-center justify-center mr-3">
                              <span className="text-lg">{getLocationIcon(item.adresse)}</span>
                            </div>
                            <div className="flex items-center space-x-2 max-w-md">
                              <button 
                                className="font-medium text-purple-600 hover:text-purple-900 hover:underline truncate"
                                title={item.adresse}
                              >
                                {getShortAddress(item.adresse)}
                              </button>
