...
export default function ExitList({ refreshFlag, onUpdate }) {
  ...
  const [exitToDelete, setExitToDelete] = useState(null)
  ...
  // Charger les mouvements "sortie"
  useEffect(() => {
    setIsLoading(true)
    api.get('/movements')
      .then(res => {
        const all = Array.isArray(res.data) ? res.data : []
        const exitsData = all.filter(m => m.type === 'sortie')
        setExits(exitsData)
        setFilteredExits(exitsData)
        setIsLoading(false)
      })
      .catch(error => {
  ...
  // Filtrer et trier les sorties
  useEffect(() => {
    let filtered = [...exits]
    
    if (filterProject) {
      filtered = filtered.filter(exit => exit.project === filterProject)
    }
  ...
  }, [exits, sortBy, sortOrder, filterProject, stockMap])

  // Fonction pour ouvrir le modal de suppression
  const handleDeleteClick = (exit) => {
  ...
  const handleDelete = async () => {
    ...
    setDeletingId(exitToDelete._id)
    try {
      await api.delete(`/movements/${exitToDelete._id}`)
      ...
      const errorInfo = handleApiError(error)
      alert(`Erreur lors de la suppression de la sortie: ${errorInfo.message}`)
    } finally {
      setDeletingId(null)
    }
  }
  ...
  if (!isLoading && filteredExits.length === 0) {
    return (
      <motion.div
      ...
      >
        ...
        <h3 className="text-xl font-semibold text-gray-700 mb-2">
          {exits.length === 0 ? 'Aucune sortie enregistrée' : 'Aucun résultat trouvé'}
        </h3>
        <p className="text-gray-500">
          {exits.length === 0 
            ? 'Commencez par ajouter votre première sortie de stock'
