...
const getStockByCategory = [celebrate(getCategorySchema), async (req, res, next) => {
  ...
  try {
    ...
    // 📊 Enregistrer début de l'opération
    const dbStartTime = Date.now();
    
    const [entries, total] = await Promise.all([
      QueryOptimizer.optimizeStockQuery(
    ...
    SystemMetrics.recordDatabaseOperation('find', {
      collection: 'StockEntry',
      duration: dbDuration,
      documentsCount: entries.length,
      indexUsed: true // Assumé avec la catégorie
    });
    
    // 📊 Enregistrer opération business
    BusinessMetrics.recordStockOperation('view_category', {
      category: categorie,
      totalValue: entries.reduce((sum, entry) => sum + (entry.prixUnitaire * entry.quantite || 0), 0),
      quantity: entries.reduce((sum, entry) => sum + (entry.quantite || 0), 0),
    ...
    logger.info(`Récupération des stocks pour la catégorie: ${categorie} (page ${page}) - ${totalDuration}ms`);
