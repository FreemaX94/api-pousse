const GoogleDriveSync = require('../services/googleDriveSync');
...
// Initialiser le service au démarrage (désactivé en développement)
if (process.env.NODE_ENV === 'production') {
  initializeSyncService().catch(error => {
    logger.error('❌ Erreur initialisation service sync:', error.message);
  });
} else {
  logger.info('ℹ️ Service Google Sheets désactivé en développement');
}
