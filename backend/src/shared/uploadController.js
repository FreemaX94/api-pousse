...
const uploadExcelFile = async (req, res) => {
  try {
    ...
    const juneSheet = sheetNames.find(name => 
      name.toLowerCase().includes('juin') || name.toLowerCase().includes('june')
    );
    const julySheet = sheetNames.find(name => 
      name.toLowerCase().includes('juillet') || name.toLowerCase().includes('july')
    );
    ...
    if (juneSheet) {
      const worksheet = workbook.Sheets[juneSheet];
      data.juin = XLSX.utils.sheet_to_json(worksheet, { header: 1 });
      // Supprimer la ligne d'en-tête
      data.juin = data.juin.slice(1);
      logger.info(`📋 Juin: ${data.juin.length} lignes extraites`);
    }

    // Traiter la feuille Juillet
    if (julySheet) {
      const worksheet = workbook.Sheets[julySheet];
      data.juillet = XLSX.utils.sheet_to_json(worksheet, { header: 1 });
      // Supprimer la ligne d'en-tête
      data.juillet = data.juillet.slice(1);
    ...
    res.json({
      ...
      filename: filename,
      sheetsFound: { juneSheet, julySheet },
      data: {
