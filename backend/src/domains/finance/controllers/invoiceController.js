...
const getInvoicesSchema = {
  [Segments.QUERY]: Joi.object({
    ...
    sortBy: Joi.string().valid('invoiceNumber', 'client.name', 'amounts.totalTTC', 'dates.issueDate', 'dates.dueDate', 'status', 'date').default('dates.issueDate'),
...
const buildInvoiceSort = (sortBy, sortOrder) => {
  ...
  // Map 'date' to 'dates.issueDate' for compatibility
  const actualSortBy = sortBy === 'date' ? 'dates.issueDate' : sortBy;
  sort[actualSortBy] = sortOrder === 'asc' ? 1 : -1;
  
  // Tri secondaire par date de création
  if (actualSortBy !== 'dates.issueDate') {
